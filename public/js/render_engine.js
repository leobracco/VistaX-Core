...
const socket = io();
window.socket = socket; // expuesto para trigger_manager.js
...
function inicializarUI() {
  ...
  sensoresOrdenados.forEach((sensor) => {
    ...
    if (isEspecial) {
      ...
      if (!monitorGrid.classList.contains("surcos-strip")) monitorGrid.classList.add("surcos-strip");
  ...
  // Notificar al layout manager que los surcos están listos
  if (window.VXLayout) window.VXLayout.onSurcosReady();
}
...
function actualizarPastillaEstado(el, data) {
  ...
  // Altura de barra vertical proporcional al SPM vs objetivo
  const objetivo  = APP_CONFIG?.setup?.densidad_objetivo || 16;
