...
socket.on("sensor_update", (data) => {
  ...
  // ============================================================
  // FIX: Calcular promedio global de s/m desde datosSurcos
...
// ============================================================
// ACTUALIZACIÓN GLOBAL (velocidad desde GPS)
// ============================================================
socket.on("global_update", (stats) => {
  ...
  // Nota: stats.promedio eliminado — se calcula en sensor_update
});
  ...
  overlay.innerHTML = `
    ...
    ">
      ...
      <div style="padding:18px;">
        ...
        <div style="background:#0a0a0a;border-radius:4px;overflow:hidden;height:6px;margin-bottom:8px;">
...
window.iniciarLoteDesdeMonitor = async function () {
  ...
  if (res.ok) {
    cerrarModalLote();
    // El footer se actualiza via lote_update socket — no hace falta tocar el DOM acá
...
window.cerrarLoteDesdeMonitor = async function () {
  ...
  if (res.ok) {
    cerrarModalLote();
    // El footer se actualiza via lote_update socket
