...
function initMQTT(io, getConfig) {
  ...
  client.on("connect", () => {
    ...
    client.subscribe("aog/field/status");       // estado campo/pintado del bridge
  ...
  client.on("message", (topic, message) => {
    try {
      ...
      // ------------------------------------------
      // 2. ESTADO CAMPO / PINTADO (del Bridge)
      // ------------------------------------------
      if (topic === "aog/field/status") {
        // Re-emitir por socket.io para que trigger_manager lo reciba
        io.emit("field_status", payload);
        return;
      }

      // ------------------------------------------
      // 2b. POSICIÓN GPS (viene del Bridge como JSON)
      // ------------------------------------------
